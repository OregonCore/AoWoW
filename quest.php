...
if(!$quest = load_cache(10, intval($id)))
{
	...
	if($quest['RequiredSkillValue']>0 && $quest['SkillOrClass']>0)
	{
		...
		*/
		...
		$quest['reqskill'] = array(
			'name' => $DB->selectCell('SELECT name FROM ?_skill WHERE skillID=?d LIMIT 1',$quest['SkillOrClass']),
	...
	if ($quest['RequiredMinRepFaction'] && $quest['RequiredMinRepValue'])
		$quest['RequiredMinRep'] = array(
			'name' => $DB->selectCell('SELECT name FROM ?_factions WHERE factionID=?d LIMIT 1', $quest['RequiredMinRepFaction']),
	...
	if ($quest['RequiredMaxRepFaction'] && $quest['RequiredMaxRepValue'])
		$quest['RequiredMaxRep'] = array(
			'name' => $DB->selectCell('SELECT name FROM ?_factions WHERE factionID=?d LIMIT 1', $quest['RequiredMaxRepFaction']),
		...
		if($quest['ReqCreatureOrGOId'.$i] != 0 && $quest['ReqCreatureOrGOCount'.$i] != 0)
		{
			...
			if ($quest['ReqSpellCast'.$i])
				$quest['coreqs'][$i]['spell'] = array(
					'name' => $DB->selectCell('SELECT spellname FROM ?_spell WHERE spellid=?d LIMIT 1', $quest['ReqSpellCast'.$i]),
	...
	if($quest['RepObjectiveFaction']>0 && $quest['RepObjectiveValue']>0)
	{
		$quest['factionreq'] = array(
			'name' => $DB->selectCell('SELECT name FROM ?_factions WHERE factionID=?d LIMIT 1', $quest['RepObjectiveFaction']),
