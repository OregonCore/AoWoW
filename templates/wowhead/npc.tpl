...
{/if} 
			...
			<div class="text">
				<a href="http://de.wowhead.com/?{$query}" target="_blank" class="button-red"><div><blockquote><i>Wowhead</i></blockquote><span>Wowhead</span></div></a>
...
{strip}
				<span id="locations">
					{foreach from=$zonedata item=zone_c name=zone_f}
						<a {if ($exdata[$smarty.foreach.zone_f.index][0].x && $exdata[$smarty.foreach.zone_f.index][0].x)}
						...
						{else}href="?zone={$zone_c.zone}">{/if}{$zone_c.name}</a>
						{if $zone_c.count>1}&nbsp;({$zone_c.count}){/if}{if $smarty.foreach.zone_f.last}.{else}, {/if}
					{/foreach}
				</span>
{/strip}
